...
export class MemoryConsolidator {
  ...
  async classify(extracted: ExtractedMemory): Promise<Result<ConsolidationDecision, EidolonError>> {
    ...
    const mergedContent = mergeContent(top.memory.content, extracted.content);
  ...
  async consolidate(
    ...
    for (const mem of extracted) {
      ...
      const applyResult = applyDecision(this.store, decision, mem, sessionId);
      if (!applyResult.ok) {
  ...
  private async detectContradiction(existing: string, incoming: string): Promise<boolean> {
    ...
    return heuristicContradiction(existing, incoming);
  }

  /**
   * Static alias for backward compatibility with tests that use
   * `MemoryConsolidator.heuristicContradiction(a, b)`.
   */
  static heuristicContradiction(a: string, b: string): boolean {
    return heuristicContradiction(a, b);
  }
}
