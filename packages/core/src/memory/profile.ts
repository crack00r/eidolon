 ...
 * recent topics, skills, decision patterns, and KG relationships.
 *
...
import { queryDevices, queryLanguages, queryRelationships, queryTimezone } from "./profile-queries.ts";
...
export interface UserProfile {
  ...
  readonly relationships: ReadonlyArray<ProfileRelationship>;
...
export class UserProfileGenerator {
  ...
  generateProfile(): UserProfile {
    ...
    const timezone = queryTimezone(this.db);
    const languages = queryLanguages(this.db);
    const devices = queryDevices(this.db);
    const relationships = queryRelationships(this.db, this.ownerName);
  ...
  private getRecentTopics(): ReadonlyArray<ProfileRecentTopic> {
    try {
      ...
      const rows = this.db
        .query(
          `SELECT tags, created_at FROM memories
           WHERE created_at > ? AND tags != '[]' ORDER BY created_at DESC`,
        )
      ...
      return [...topicLastSeen.entries()]
        .sort((a, b) => b[1] - a[1])
        .slice(0, MAX_RECENT_TOPICS)
        .map(([topic, lastMentioned]) => ({ topic, lastMentioned }));
    } catch (err) {
  ...
  private getSkills(): ReadonlyArray<ProfileSkill> {
    try {
      ...
      return rows.map((row) => ({ name: extractSkillName(row.content), level: inferSkillLevel(row.content) }));
    } catch (err) {
  ...
  private getDecisionPatterns(): ReadonlyArray<ProfileDecisionPattern> {
    try {
      const rows = this.db
        ...
        .all(MAX_DECISION_PATTERNS * 5) as ContentConfidenceRow[];
      ...
      return [...groups.values()]
        ...
        .map(({ pattern, count }) => ({ pattern, examples: count }));
    } catch (err) {
